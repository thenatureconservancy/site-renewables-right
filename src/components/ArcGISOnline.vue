...
function getOrganizationName(orgId, token = null) {
...
function searchPortal() {
  ...
  portal.load().then(() => {
    const queryParams = new PortalQueryParams({
      query: `(title:"${searchTerm.value}" OR tags:"${searchTerm.value}*" OR description:"${searchTerm.value}") AND (type:"Feature Service" OR type:"Image Service") AND access:public AND contentstatus:public_authoritative`,
      sortField: 'num_views',
      sortOrder: 'asc',
      num: 10,
    })
    portal.queryItems(queryParams).then((results) => {
      ...
      results.results.forEach((item) => {
        ...
        getOrganizationName(orgId).then((orgName) => {
          console.log(item)
  ...
  <q-card rounded style="width: 600px">
    ...
    <q-card-section class="q-pt-none">
      <q-input
        ...
        debounce="500"
        v-model="searchTerm"
        @update:model-value="searchPortal()"
      ...
      <div class="row items-center justify-center q-mb-sm">
        <div>
          <p class="text-body1 text-weight-medium q-mb-none">
            <q-icon name="search" class="" color="blue-grey-9"></q-icon>&nbsp;Search Results
          </p>
        </div>
        <q-space></q-space>
        <q-btn
        ...
        <q-item v-for="layer in searchResults" :key="layer.title">
          ...
          <q-item-section side>
            <q-btn
              ...
              round
              icon="add"
              @click="mapLayers.push(layer); addLayerToMap(layer)"
            >
              <q-tooltip
                class="text-body2 bg-white text-blue-grey-9"
    ...
    <q-card-section class="q-pt-none">
      <p class="text-body1 text-weight-medium q-mb-sm">
        <q-icon color="blue-grey-9" name="star" class="q-mb-xs"></q-icon>&nbsp;Suggested Layers
    ...
    <q-card-section class="q-pt-none" v-if="mapLayers.length > 0">
      <p class="text-body1 q-mb-sm text-weight-medium">
        <q-icon name="settings" color="blue-grey-9" class="q-mb-xs"></q-icon> Manage Layers
      </p>
      <q-list bordered padding class="bg-white rounded">
