...
function searchPortal() {
  ...
  portal.load().then(() => {
    const queryParams = new PortalQueryParams({
      query: `(title:"${searchTerm.value}" OR tags:"${searchTerm.value}*" OR description:"${searchTerm.value}") AND type:"Feature Service" AND access:public AND contentstatus:public_authoritative`,
      sortField: 'num_views',
      sortOrder: 'asc',
      num: 10,
    })
