...
<template>
  <div v-if="mapStore.results.length > 0">
    ...
    <q-expansion-item
      ...
      >
      <q-list bordered="">
        <p class="q-ml-md q-mb-sm q-mt-sm text-body2 text-caption">Features: {{ mapStore.getCounts().avoid }}</p>
      <div v-for="(item, index) in mapStore.results" :key="index">
        <q-item class="" v-if="item.totalArea !== 0 && item.map !== 'opportunities' && item.layerName !== 'Landscape Connectivity'">
    ...
    <q-expansion-item
      ...
      >
    <q-list bordered="">
      <p class="q-ml-md q-mb-sm q-mt-sm text-caption">Features: {{mapStore.getCounts().minimize}}</p>
      <div v-for="(item, index) in mapStore.results" :key="index">
        <q-item class="" v-if="item.layerName === 'Landscape Connectivity' && item.totalArea !== 0">
          <q-item-section class="col-4" :style="{ background: item.color }">
            <q-item-label class="text-blue-grey-9 text-body2 q-pa-md text-bold">
              {{ item.layerName }}</q-item-label
            >
    ...
    <q-expansion-item
        ...
        default-opened=""
      >
      <p class="q-ml-md q-mb-sm q-mt-sm text-caption">Features: {{ mapStore.getCounts().develop }}</p>
      <div v-for="(item, index) in mapStore.results" :key="index">
  ...
  <q-separator></q-separator>
    <q-expansion-item
        ...
        default-opened=""
      >
      <p class="q-ml-md q-mb-none q-mt-none text-body2 text-caption">Features: {{ mapStore.getCounts().review }}</p>
    <q-list class="q-pl-md" bordered="" dense>
      
      <div v-for="(item, index) in mapStore.results" :key="index">
        <q-item class="" v-if="item.totalArea == 0 && item.map !== 'opportunities'&& item.layerName !== 'Landscape Connectivity'">
          <q-item-section>
            <q-item-label class="text-white text-body2 ">
              {{ item.layerName }}</q-item-label
            >
          </q-item-section>
          <q-item-section side :style="{ background: item.color }">
  ...
  >
  <div class="text-center q-mb-md"><q-icon name="o_pin_drop" size="xl" class="text-center"></q-icon></div>
   <p class="q-pb-xl q-mb-xl"> Click anywhere on the map to generate a Site Renewables Right area summary</p>
